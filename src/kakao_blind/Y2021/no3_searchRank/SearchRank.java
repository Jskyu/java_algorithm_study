package kakao_blind.Y2021.no3_searchRank;
...
public class SearchRank {
    ...
    public int[] solution2(String[] infos, String[] queries) {
        ...
        for (int i = 0; i < queries.length; i++) {
            String[] splitQuery = queries[i].replaceAll("and ", "").split(" ");

            for (String info : infos) {
                boolean isT = true;
                for (int j = 0; j < 5; j++) {
                    if (splitQuery[j].equals("-")) continue;
                    if (j == 4) {
                        int queryScore = Integer.parseInt(splitQuery[4]);
                        int infoScore = Integer.parseInt(info.split(" ")[4]);
                        if (queryScore > infoScore) {
                            isT = false;
                        }
                    } else if (!info.contains(splitQuery[j])) {
                        isT = false;
                        break;
                    }
                }

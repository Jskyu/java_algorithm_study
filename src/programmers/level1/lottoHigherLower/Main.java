package programmers.level1.lottoHigherLower;
    ...
    public static int[] solution(int[] lottos, int[] win_nums) {
        ...
        if (zeroCount == 0) {
            ...
            for (int i = 0; i < lottos.length; i++) {
                if (lottos[i] == win_nums[i]) value++;
            }
        } else {
            for (int winNum : win_nums) {
                for (int lottoNum : lottos) {
                    if (winNum == lottoNum) value++;
                }
            }
        }

        answer[0] = getRank(value + zeroCount);
        answer[1] = getRank(value);

        return answer;
    }

    private static int getRank(int value) {
        switch (value) {
            case 2:
                return 5;
            case 3:

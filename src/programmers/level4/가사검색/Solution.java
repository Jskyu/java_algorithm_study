...
class Solution {
    public int[] solution(String[] words, String[] queries) {
        ...
        try {
            ...
            Trie trie = new FrontTrie();
            Trie sufTrie = new BackTrie();
            Map<String, Integer> resultMap = new HashMap<>();

            Arrays.stream(words).forEach(trie::insert);
            Arrays.stream(words).forEach(sufTrie::insert);
            ...
            Arrays.stream(queries)
                ...
                if (query.matches("[?]+")) {// ?로만 구성된 검색키워드
                    ...
                    resultMap.put(query, sufTrie.getCount(query.substring(query.lastIndexOf("?") + 1), query.length()));
                } else {// ?로 끝나는 검색 키워드
                    resultMap.put(query, trie.getCount(query.substring(0, query.indexOf("?")), query.length()));
                }
            });
    ...
    interface Trie {
        ...
        int getChildNodeCount(Node node, int len);
    }
    ...
    static class FrontTrie implements Trie {
        ...
        @Override
        public int getChildNodeCount(Node node, int len) {
            int count = 0;
            if (len <= 0) {
